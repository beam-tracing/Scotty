...
def check_vector_pointing_into_plasma(q_X: float, q_Y: float, q_Z: float, vector: FloatArray, field: MagneticField_3D_Cartesian):
    ...
    q_XYZ = [q_X, q_Y, q_Z]
    q_XYZ_minus = q_XYZ - 0.01*unitvector
    q_XYZ_plus  = q_XYZ + 0.01*unitvector

    polflux_at_q_minus = field.polflux(*q_XYZ_minus)
    polflux_at_q =       field.polflux(*q_XYZ)
    polflux_at_q_plus =  field.polflux(*q_XYZ_plus)
