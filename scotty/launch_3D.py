...
class LaunchParameters:
    r"""
    Calculates the launch parameters
    Expand a bit more
    ...
    def __init__(
        ...
        Psi_BC_flag: Union[bool, str, None],
        ...
        self.launch_angular_freq = freq_GHz_to_angular_frequency(self.launch_freq_GHz)
        self.launch_wavenumber = angular_frequency_to_wavenumber(self.launch_angular_freq)
    ...
    def calculate_entry_position(self, field: MagneticField_3D_Cartesian, boundary_adjust: float = 1e-6):       
        if hasattr(self, "q_initial_cartesian"): return self.q_initial_cartesian
        else:
            ...
            X_start, Y_start, Z_start = self.q_launch_cartesian
            ...
            poloidal_launch_angle = -np.deg2rad(self.poloidal_launch_angle_deg_Torbeam)
            toroidal_launch_angle =  np.deg2rad(self.toroidal_launch_angle_deg_Torbeam) + np.pi
            # TO REMOVE -- temporarily disabling
            # poloidal_launch_angle = self.poloidal_launch_angle_rad
            # toroidal_launch_angle = self.toroidal_launch_angle_rad
            ...
            def ray_line(tau):
                ...
                return self.q_launch_cartesian + tau * XYZ_array
            ...
            def poloidal_flux_boundary_along_ray_line(tau):
                ...
                return polflux - self.poloidal_flux_enter
            ...
            if field.polflux(X_boundary, Y_boundary, Z_boundary) > self.poloidal_flux_enter: X_boundary, Y_boundary, Z_boundary = ray_line(boundary_tau + boundary_adjust)
            
            # Saving the entry point in the main class
            self.q_initial_cartesian = np.array([X_boundary, Y_boundary, Z_boundary])

            # Now, we also perform auto_delta_sign checks (if the user specifies)
            if self.auto_delta_sign:
                if field.d_polflux_dX(*self.q_initial_cartesian, self.delta_X) > 0: self.delta_X = -1 * self.delta_X
                if field.d_polflux_dY(*self.q_initial_cartesian, self.delta_Y) > 0: self.delta_Y = -1 * self.delta_Y
                if field.d_polflux_dZ(*self.q_initial_cartesian, self.delta_Z) > 0: self.delta_Z = -1 * self.delta_Z
    
    def calculate_beam_launch(self, field: MagneticField_3D_Cartesian, hamiltonian: Hamiltonian_3D, temperature = None):
        # TO REMOVE -- not supposed to be _Torbeam???
        poloidal_launch_angle = self.poloidal_launch_angle_rad_Torbeam
        toroidal_launch_angle = self.toroidal_launch_angle_rad_Torbeam
        q_launch_cartesian = self.q_launch_cartesian
        q_initial_cartesian = self.q_initial_cartesian
        K0 = self.launch_wavenumber

        # Finding K_launch
        K_X_launch = -K0 * np.cos(toroidal_launch_angle) * np.cos(poloidal_launch_angle)
        K_Y_launch = -K0 * np.sin(toroidal_launch_angle) * np.cos(poloidal_launch_angle)
        K_Z_launch = -K0 * np.sin(poloidal_launch_angle)
        ...
        diag = K0*self.launch_beam_curvature + 2j/self.launch_beam_width**2
        ...
        if not self.vacuum_propagation_flag:

            # Saving all the main calculations in the main class
            self.K_launch_cartesian = K_launch_cartesian
            self.K_initial_cartesian = K_launch_cartesian
            self.Psi_3D_launch_labframe_cartesian = Psi_3D_launch_labframe_cartesian
            self.Psi_3D_entry_labframe_cartesian = None
            self.Psi_3D_initial_labframe_cartesian = Psi_3D_launch_labframe_cartesian
            self.distance_from_launch_to_entry = None
        ...
        # Saving all the main calculations in the main class
        self.K_launch_cartesian = K_launch_cartesian
        self.K_initial_cartesian = K_initial_cartesian
        self.Psi_3D_launch_labframe_cartesian = Psi_3D_launch_labframe_cartesian
        self.Psi_3D_entry_labframe_cartesian = Psi_3D_entry_labframe_cartesian
        self.Psi_3D_initial_labframe_cartesian = Psi_3D_initial_labframe_cartesian
        self.distance_from_launch_to_entry = distance_from_launch_to_entry
...
# TO REMOVE -- leaving this here just in case
...
#     )
#     distance_from_launch_to_entry = np.linalg.norm(q_launch_cartesian - q_initial_cartesian)
#     Psi_w_inverse_entry_beamframe_cartesian = distance_from_launch_to_entry / wavenumber_K0 * np.eye(2) + Psi_w_inverse_launch_beamframe_cartesian
