...
def hot_dielectric(dt, Te_along_beam, m_range, omega):
    ...
    v_te = np.sqrt(
        2 * Te_along_beam / m_e - 0.5 * (2 * Te_along_beam / m_e) ** 2 / c**2
    )  # relativistic correction considered
    ...
    def zeta(
        m,
    ):  # wave frequency after doppler shift due to electron thermal speed, scaled by Vte
    ...
    def m_bessel(m, b_s):
        real = scipy.integrate.quad(real_integrand, -np.pi, np.pi, args=(b_s, m))[0]
        imag = scipy.integrate.quad(imag_integrand, -np.pi, np.pi, args=(b_s, m))[0]
        return real + 1j * imag
    ...
    for m in range(-m_range, m_range + 1):
        ...
        epsi[m][0][0] += (
            C1 * m**2 * I / b_s * np.exp(-b_s) * plasma_dispersion_func(zeta(m))
        )
        epsi[m][0][1] += (
            1j * C1 * m * (dI_db - I) * np.exp(-b_s) * plasma_dispersion_func(zeta(m))
        )
        epsi[m][0][2] += (
            -C2
            * np.sqrt(2 / b_s)
    ...
    Epsi_cold[0, 1, :] = -1j * omega_pe**2 * omega_ce / omega / (omega**2 - omega_ce**2)
    Epsi_cold[1, 0, :] = 1j * omega_pe**2 * omega_ce / omega / (omega**2 - omega_ce**2)
...
def xy_to_u_basis_change(dt, data, data_type):
    ...
    if data_type == "vector":
        ...
        if np.shape(data) != (L, 3):
            raise TypeError("Wrong vector shape, required vector shape (len(tau), 3)")
        result = np.zeros(np.shape(data)) + 0j
        for i in range(L):
            result[i, :] = np.matmul(M[:, :, i], data[i, :].real) + 1j * np.matmul(
                M[:, :, i], data[i, :].imag
            )
        return result
...
def absorption_coef(e_hat, Epsi_a):
    ...
    for i in range(np.shape(e_hat)[0]):
        gamma.append(np.dot(np.matmul(Epsi_a[:, :, i], e_hat[i]), e_hat[i].conj()))
...
def power_along_beam(dt, gamma, P0):
    ...
    P_sol = scipy.integrate.solve_ivp(
        dP_dtau,
        [0, len(tau) - 1],
        [P0],
        method="LSODA",
        t_eval=np.arange(0, len(tau), 1),
    )
...
def Te_along_beam_interp(Te_filename, dt):
    ...
    # interpolation
    Te_spl = UnivariateSpline(Te_data_radialcoord_array, Te_data_density_array)
...
def power_deposition_Gaussian(
    dt, P_along_beam, gamma, beam_plus_flux, beam_minus_flux, len_rho=1000
):
    poloidal_flux = dt.analysis.poloidal_flux.data
    tau = dt.analysis.tau.data
    ...
    def dV_drho_circular(rho):
        ...
        return (
            (4 * np.pi**2 * dt.inputs.minor_radius_a.data * rho * dt.inputs.R_axis.data)
        ).reshape((len_rho, 1))
    ...
    integrated_Gaussian = cumulative_trapezoid(
        np.transpose(Gaussian_absorption(rho) * dV_drho_circular(rho)), rho
    )

    dPi = (
        P_along_beam[start_truncate:end_truncate]
        * gamma[start_truncate:end_truncate]
        * 2
        * tau[-1]
        / len(tau)
    )
    Ci = dPi / integrated_Gaussian[:, -1]
    dPi_dV = np.reshape(np.sum(Ci * Gaussian_absorption(rho), axis=1), (len_rho, 1))
    return dPi_dV * dV_drho_circular(rho)


def power_deposition_integration(
    dt, P_along_beam, gamma, R_step=0.001, theta_step=0.1, len_rho=1000
):
    ...
    width_x = beam_width(
        dt.analysis.g_hat, np.array([0.0, 1.0, 0.0]), dt.analysis.Psi_3D
    ).data
        ...
        if dP_dtau[i] > 1e-2 / len(tau):  # skip insignificant cases
            ...
            for j in range(int(R_bound[i] / R_step)):
                ...
                for k in range(int(2 * np.pi / theta_step)):
                    ...
                    W = R_length * (
                        np.cos(theta) * width_x_uvec[i, :]
                        + np.sin(theta) * width_y_uvec[i, :]
                    )
                    coord = beam[i, :] + W
            ...
            P_deposited = (
                P_deposited
                + P_deposited_step / dP_cross_sect * dP_dtau[i] * len_rho * dtau[i]
            )
...
def heat_me_up(
    ...
    # calculation
    Epsi_cold, Epsi_h, Epsi_a = hot_dielectric(dt, Te_along_beam, m_range, omega)
    ...
    if Integration_method_flag:
        P_deposited_integration = power_deposition_integration(
            dt,
            P_along_beam,
            gamma,
            R_step=R_step,
        ...
        print(
            "Integration method total power deposited: ",
            np.sum(P_deposited_integration) / len(P_deposited_integration),
        )

    # check result: full absorption should give 1 for both cases
    print(
        "Gaussian method total power deposited: ",
        np.sum(P_deposited_Gaussian) / len(P_deposited_Gaussian),
    )
    ...
    if figure_flag == True:
        ...
        plt.plot(
            rho,
            P_deposited_Gaussian,
            label="power deposition Gaussian (power againt rho)",
        )
        if Integration_method_flag:
            plt.plot(
                rho,
                P_deposited_integration,
                label="power deposition Integration (power againt rho)",
            )
        plt.plot(
            np.sqrt(dt.analysis.poloidal_flux.data),
            tau / tau.max(),
            "--",
            label="beam trajectory (normalised tau against rho)",
        )
