...
def make_solver_events(
    ...
    def event_leave_plasma(tau, ray_parameters_3D, hamiltonian: Hamiltonian_stellarator):
        q_X, q_Y, q_Z, K_X, K_Y, K_Z = ray_parameters_3D
        rtz_point = np.array(xyz2rtz(equilibrium, q_X, q_Y, q_Z))
        polflux = rtz_point[0]
    ...
    def event_leave_LCFS(tau, ray_parameters_3D, hamiltonian: Hamiltonian_stellarator):
        q_X, q_Y, q_Z, K_X, K_Y, K_Z = ray_parameters_3D
        rtz_point = np.array(xyz2rtz(equilibrium, q_X, q_Y, q_Z))
        polflux = rtz_point[0]
        polflux_LCFS = 1.0
    ...
    # hopefully this isnt needed because I cant properly implement this for stellarators
    # Capture the bounding box of the magnetic field
    # To see if the ray has left the plasma
    X_coord_min, X_coord_max =    0, 6.2 # guesstimation based on DESC plot_surfaces for W7-X example
    Y_coord_min, Y_coord_max = -6.2, 6.2 # guesstimation based on DESC plot_surfaces for W7-X example
    Z_coord_min, Z_coord_max =   -1,   1 # guesstimation based on DESC plot_surfaces for W7-X example
    ...
    @_event(terminal = True, direction = -1.0)
    def event_leave_simulation(tau, ray_parameters_3D, hamiltonian: Hamiltonian_stellarator):
        q_X, q_Y, q_Z, K_X, K_Y, K_Z = ray_parameters_3D
    ...
    def event_cross_resonance(tau, ray_parameters_3D, hamiltonian: Hamiltonian_stellarator):
        q_X, q_Y, q_Z, K_X, K_Y, K_Z = ray_parameters_3D
        rtz_point = np.array(xyz2rtz(equilibrium, q_X, q_Y, q_Z))
        rtz_grid = Grid(rtz_point)
        B = equilibrium.compute(names=("B"), grid=rtz_grid)
    ...
    @_event(terminal = True, direction = 0.0)
    def event_cross_resonance2(tau, ray_parameters_3D: float, hamiltonian: Hamiltonian_stellarator):
        q_X, q_Y, q_Z, K_X, K_Y, K_Z = ray_parameters_3D
        rtz_point = np.array(xyz2rtz(equilibrium, q_X, q_Y, q_Z))
        rtz_grid = Grid(rtz_point)
        B = equilibrium.compute(names=("B"), grid=rtz_grid)
    ...
    @_event(terminal = False, direction = 1.0)
    def event_reach_K_min(tau, ray_parameters_3D, hamiltonian: Hamiltonian_stellarator):
        q_X, q_Y, q_Z, K_X, K_Y, K_Z = ray_parameters_3D
...
def ray_evolution_stellarator_fun(tau, ray_parameters_3D, hamiltonian: Hamiltonian_stellarator):
...
def propagate_ray_stellarator(
    ...
    solver_ray_events = make_solver_events(
        poloidal_flux_enter, launch_angular_frequency, equilibrium
    )
