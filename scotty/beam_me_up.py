...
def beam_me_up(
    ...
    #
    ##################################################
    # q_launch_cylindrical = launch_position # This is left here for backward compatibility
    # params = Par
    ...
    # if ne_data_path == pathlib.Path("."): pass
    if eduard_2d_ne_flag:
    ...
    if eduard_temporary_ray_tracing_flag:
        ...
        tau_array, output_arrays, solver_status = propagate_ray(
            ...
            False, # quick_run
            len_tau,
            eduard_temporary_ray_tracing_flag = eduard_temporary_ray_tracing_flag # TO REMOVE
        )
        ...
        K_cutoff = K_magnitude_array[cutoff_index]
        Kc_K0 = np.abs(K_cutoff / hamiltonian.wavenumber_K0)

        sin_theta_m = (b_hat_R*K_R_array + b_hat_zeta*K_zeta_initial/q_R_array + b_hat_Z*K_Z_array) / K_magnitude_array
        ...
        if hasattr(field, "n_e_grid"):
            epsilon = DielectricTensor(
                field.electron_density(q_R_array, q_Z_array),
                launch_angular_frequency,
                B_magnitude,
            )
        else:
        ...
        for i, ax in enumerate(axes):
            ...
            start_R, _, start_Z = launch_position
            ax.scatter(start_R, start_Z, label="Launch Position", color="red", marker=">")
            ax.plot([start_R, q_R_array[0]], [start_Z, q_Z_array[0]], color="black", linestyle=":")
            ax.plot(q_R_array, q_Z_array, label="Central Ray", color="black", linestyle="-")
            ...
            if i == 0:
                ...
                ax.scatter(q_R_array[cutoff_index], q_Z_array[cutoff_index], label="Cutoff", color="red", marker="x")
                ax.legend()
                ax.set_xlim(q_R_min - padding*R_range - 0.1, q_R_max + padding*R_range + 0.1)
                ax.set_ylim(q_Z_min - padding*Z_range - 0.1, q_Z_max + padding*Z_range + 0.1)
                ax.set_title("Closer View")
            else:
                # Zoomed-in view
                ax.scatter(q_R_array[cutoff_index], q_Z_array[cutoff_index], label="Cutoff", color="red", marker="x")
                # ax.legend()
                ax.set_xlim(q_R_min - padding*R_range - 0.01, q_R_max + padding*R_range + 0.01)
                ax.set_ylim(q_Z_min - padding*Z_range - 0.01, q_Z_max + padding*Z_range + 0.01)
                ax.set_title("Zoomed View")
        ...
        plt.suptitle(f"Ray Path Plots for \n{launch_freq_GHz} GHz, pol {poloidal_launch_angle_Torbeam} deg, tor {toroidal_launch_angle_Torbeam} deg")
        plt.tight_layout()
        plt.savefig(output_path / f"ray_path{output_filename_suffix}.png", bbox_inches="tight", dpi=600)
        plt.draw()
        plt.close()
        ...
        print()
        print()
        print(f"RUN DONE for frequency {launch_freq_GHz}, pol = {poloidal_launch_angle_Torbeam}, tor = {toroidal_launch_angle_Torbeam}")
        print()
        print()
        print()
...
def default_plots(
    ...
    from scotty.plotting import plot_toroidal_beam_path, plot_widths

    print("Making figures")
    plot_poloidal_beam_path(dt, filename=(output_path / f"beam_path{suffix}_zoomed.png"), zoom=True)
    plot_poloidal_beam_path(dt, filename=(output_path / f"beam_path{suffix}.png"), zoom=False)
    plot_toroidal_beam_path(dt, filename=(output_path / f"beam_path_toroidal.png"))
    plot_dispersion_relation(dt.analysis, filename=(output_path / f"H_{suffix}.png"))
    plot_widths(dt, filename=(output_path / f"beam_widths.png"))
...
def create_magnetic_geometry(
    ...
    **kwargs,
    ...
    if find_B_method == "torbeam":
        ...
        # if "n_e" in data.keys(): args["n_e"] = torbeam.n_e
        if eduard_2d_ne_flag: args["n_e"] = torbeam.n_e
        ...
        # if "n_e" in data.keys(): args["n_e"] = unflatten(data["n_e"])
        if eduard_2d_ne_flag: args["n_e"] = unflatten(data["n_e"])
        return InterpolatedField(**args)
