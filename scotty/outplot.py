...
class outplot(object):
    ...
    def __init__(self, input_path="", suffix="", output_path="", relativistic_flag=False):
        # Path specifics
...
def compare_plots(
    plotobject_1: outplot, 
    plotobject_2: outplot, 
    name_tuple: Tuple[str, ...],
    option=1):
        """
        Plotting function for simultaneously plotting two outplot objects. Currently WIP.
        """
                
        if option == 0:
            for opt in range(1, 16):
                compare_plots(plotobject_1, plotobject_2, option=opt)
            ...
            width_RZ_1 = np.sqrt(
                ...
                / np.imag(
                    contract_special(
                        W_uvec_RZ_1, contract_special(plotobject_1.Psi_3D_output, W_uvec_RZ_1)
                    )
                )
            )
            ...
            width_RZ_2 = np.sqrt(
                ...
                / np.imag(
                    contract_special(
                        W_uvec_RZ_2, contract_special(plotobject_2.Psi_3D_output, W_uvec_RZ_2)
                    )
                )
            )
            ...
            object_dict = {
                'plotobject_1': plotobject_1,
                'plotobject_2': plotobject_2
            }
            colour_tup = ('red', 'blue')
            for i in (1, 2):
                name = name_tuple[i-1]
                colour = colour_tup[i-1]
                plotobject = object_dict[f"plotobject_{i}"]
                ...
                plt.plot(
                    plotobject.q_R_array[: plotobject.out_index], plotobject.q_Z_array[: plotobject.out_index], label=name, c=colour, linewidth=0.5
                )
                plt.plot(
                    [plotobject.launch_position[0], plotobject.q_R_array[0]],
                    [plotobject.launch_position[2], plotobject.q_Z_array[0]],
                    ":k", c=colour, linewidth=0.5
                )
                plt.plot(
            ...
            #plt.gca().set_aspect("equal", adjustable="box")
            plt.savefig(self.output_path + "Psi_M_Output.jpg", dpi=200)
            ...
            plt.plot(self.l_lc, self.kappa_dot_yhat_output)
            plt.title(r"$\kappa \cdot \hat{y}$")
            plt.savefig(self.output_path + "kappa_dot_yhat_output.jpg", dpi=200)
            ...
            plt.savefig(self.output_path + "option13.jpg", dpi=200)
            ...
            plt.xlabel(r"$(l - l_c)$")
            plt.ylabel("deg")
            plt.savefig(self.output_path + "option14.jpg", dpi=200)
