...
class outplot(object):
    ...
    def __init__(self, input_path = '', suffix = '', output_path = ''):
        ...
        self.xhat_dot_grad_bhat_dot_xhat_output = loadfile["xhat_dot_grad_bhat_dot_xhat_output"]
        self.xhat_dot_grad_bhat_dot_ghat_output = loadfile["xhat_dot_grad_bhat_dot_ghat_output"]
        self.yhat_dot_grad_bhat_dot_ghat_output = loadfile["yhat_dot_grad_bhat_dot_ghat_output"]
        ...
        # Other values
        self.l_lc = (self.distance_along_line - self.distance_along_line[self.cutoff_index])  # Distance from cutoff
        [self.q_X_array, self.q_Y_array, self.q_Z_array] = find_q_lab_Cartesian(np.array([self.q_R_array, self.q_zeta_array, self.q_Z_array]))
    ...
    def plotout(self, option=0):
        ...
        if option==0:
            ...
            width_RZ = np.sqrt(2/np.imag(contract_special(W_uvec_RZ, contract_special(self.Psi_3D_output, W_uvec_RZ))) )
            ...
            plt.plot(self.q_R_array[:self.out_index], self.q_Z_array[:self.out_index], "k")
            plt.plot([self.launch_position[0], self.q_R_array[0]], [self.launch_position[2], self.q_Z_array[0]], ":k")
            plt.plot(W_line_RZ_1_Rpoints[:self.out_index], W_line_RZ_1_Zpoints[:self.out_index], "k--")
            plt.plot(W_line_RZ_2_Rpoints[:self.out_index], W_line_RZ_2_Zpoints[:self.out_index], "k--")
            ...
            R_outboard = self.R_midplane_points[find_nearest(self.poloidal_flux_on_midplane[index_polmin:], 1) + index_polmin]
            index_local_polmax = find_nearest(self.poloidal_flux_on_midplane[0: index_polmin], 10)
            R_inboard = self.R_midplane_points[find_nearest(self.poloidal_flux_on_midplane[index_local_polmax:index_polmin], 1) + index_local_polmax]
            ...
            launch_position_X, launch_position_Y, launch_position_Z = find_q_lab_Cartesian(self.launch_position)
            entry_position_X, entry_position_Y, entry_position_Z = find_q_lab_Cartesian(np.array([self.q_R_array[0], self.q_zeta_array[0], self.q_Z_array[0]]))
            ##
            ## For plotting the width in the XY plane
            g_hat_Cartesian = np.zeros([self.out_index, 3])
            g_hat_Cartesian[:,0] = self.g_hat_output[:,0] * np.cos(self.q_zeta_array) - self.g_hat_output[:,1] * np.sin(self.q_zeta_array)
            g_hat_Cartesian[:,1] = self.g_hat_output[:,0] * np.sin(self.q_zeta_array) + self.g_hat_output[:,1] * np.cos(self.q_zeta_array)
            g_hat_Cartesian[:,2] = self.g_hat_output[:,2]

            W_uvec_XY = make_unit_vector_from_cross_product(g_hat_Cartesian, np.array([0,0,1]))
            width_XY = np.sqrt(2/ np.imag(contract_special(W_uvec_XY, contract_special(self.Psi_3D_Cartesian, W_uvec_XY))))
            ...
            plt.plot(self.q_X_array[: self.out_index], self.q_Y_array[:self.out_index], "k")
            plt.plot(
                [launch_position_X, entry_position_X],
                [launch_position_Y, entry_position_Y],
                ":k",
            )
            plt.plot(W_line_XY_1_Xpoints[: self.out_index], W_line_XY_1_Ypoints[: self.out_index], "r--")
            plt.plot(W_line_XY_2_Xpoints[: self.out_index], W_line_XY_2_Ypoints[: self.out_index], "g--")
            ...
            plt.plot(
                self.l_lc,
                (np.sin(self.theta_output) / self.g_magnitude_output) * self.d_theta_d_tau,
            ...
            plt.plot(
                self.l_lc,
                (np.sin(self.theta_output) * np.tan(self.theta_output) / self.g_magnitude_output) * self.d_xhat_d_tau_dot_yhat_output,
            ...
            plt.plot(self.l_lc, np.rad2deg(self.delta_theta_m), label=r"$\Delta \theta_m$")
            plt.legend()
            plt.xlabel(r"$(l - l_c)$")
            plt.ylabel("deg")
            ...
            om_pe_norm = find_normalised_plasma_freq(self.electron_density_output, launch_angular_frequency)
            om_ce_norm = find_normalised_gyro_freq(self.B_magnitude, launch_angular_frequency)
            ...
            factor2 = (self.epsilon_perp_output**2 - self.epsilon_g_output**2 - self.epsilon_perp_output * self.epsilon_para_output - self.epsilon_perp_output * N_sq + self.epsilon_para_output * N_sq) / (-(self.epsilon_perp_output**2) + self.epsilon_g_output**2 - self.epsilon_perp_output * self.epsilon_para_output + self.epsilon_perp_output * N_sq * 2)
