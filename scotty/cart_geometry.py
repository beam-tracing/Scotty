...
# change the Y to be the B field 
    def __init__(self, B_Y_0: float, R_axis: float, minor_radius_a: float,grid_buffer_factor:float,):
    ...
    def B_Y(self, q_X: ArrayLike, q_Y: ArrayLike, q_Z: ArrayLike) -> FloatArray:
        q_X, q_Y, q_Z = np.asfarray(q_X), np.asfarray(q_Y), np.asfarray(q_Z)
        return np.ones_like(q_Y) * self.B_Y_0
        # B_Y is constant
    ...
    def poloidal_flux(self, q_X: ArrayLike, q_Y: ArrayLike, q_Z: ArrayLike) -> FloatArray:
        return q_X*1/self.minor_radius_a
        # linear in x 