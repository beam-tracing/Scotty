...
class SweepDataset:
    ...
    def generate_opt_tor(self):
        ...
        if 'opt_tor' in self.get_Variables():
            ...
            Poloidal_Angles = self.get_Coordinate_array('cutoff_theta_m', 'poloidal_angle')
            ...
            for frequency in Frequencies:
                for poloidal_angle in Poloidal_Angles:
                    ...
                    spline = self.create_1Dspline('cutoff_theta_m', 'toroidal_angle', coords_dict)
    ...
    def generate_reflection_mask(self):
        ...
        if 'reflection_mask' in self.get_Variables():
            ...
            Poloidal_Angles = self.get_Coordinate_array('distance_along_line', 'poloidal_angle')
            Toroidal_Angles = self.get_Coordinate_array('distance_along_line', 'toroidal_angle')
    ...
    def get_simulated_power_prof(self, frequency, poloidal_angle, noise_flag = True, std = 0.05):
        ...
        theta_m_spline = self.create_1Dspline('cutoff_theta_m', 'toroidal_angle', coords_dict)
        delta_spline = self.create_1Dspline('cutoff_delta_theta_m', 'toroidal_angle', coords_dict)
    ...
    def create_2Dspline(self, variable, xdimension, ydimension, coords_dict):
        ...
        if args in self.spline_memo.keys():
            ...
            spline = RectBivariateSpline(x=x_coordinates, y=y_coordinates, z=z_values, s=0)
            self.spline_memo[args] = spline
            return spline
    ...
    def plot_cutoff_contour(
        ...
        if const_angle_str == 'toroidal_angle':
            ...
            raise ValueError("var_angle_str must be 'toroidal_angle' or 'poloidal_angle'.")
        ...
        spline_surface = self.create_2Dspline(variable, xdimension, ydimension, coords_dict)
        ...
        reflection_mask = self.dataset['reflection_mask'].loc[coords_dict].transpose(xdimension, ydimension).values
        mask_function = RegularGridInterpolator((x_coordinates, y_coordinates), reflection_mask, method="nearest", bounds_error=False)
        interp_mask = mask_function((Xgrid, Ygrid))
        Z_masked = np.ma.array(Zgrid, mask=interp_mask)

        fig, ax = plt.subplots()
        if mask_flag:
            CS = ax.contour(Xgrid, Ygrid, Z_masked, levels=20, cmap='plasma_r', corner_mask=True)
        else:
            CS = ax.contour(Xgrid, Ygrid, Zgrid, levels=20, cmap='plasma_r', corner_mask=True)
        ...
        fig.colorbar(CS)
        if save_path:
            plt.savefig(save_path + f'cutoff_contour_{const_angle_str}{const_angle}.jpg', dpi=200)       
