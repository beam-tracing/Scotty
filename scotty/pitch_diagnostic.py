...
from scotty.sweep_analysis import SweepDataset, fit_gaussian, gaussian, noisy_gaussian, find_nearest, scale_array
...
class PitchDiagnostic:
    ...
    def simulate_loc_measurements(self, SweepDataset, iterations=500, std_noise=None):
        ...
        for combination in product(frequencies, poloidal_angles):
            ...
            self.ds_dict[descriptor]["loc_product_profile"].loc[coords_dict] = loc_product_signal
            ...
            for iteration in iter_array:
                ...
                loc_m_measurement =  scale_array(loc_m_array) + np.random.normal(0, std_noise, loc_m_array.shape)
                loc_product_measurement = scale_array(loc_product_array) + np.random.normal(0, std_noise, loc_product_array.shape)
                index = {
                    'frequency':frequency,
                    'poloidal_angle':poloidal_angle,
                    "iteration": iteration,
                }
                self.ds_dict[descriptor]['simulated_loc_m'].loc[index] = loc_m_measurement
                self.ds_dict[descriptor]['simulated_loc_product'].loc[index] = loc_product_measurement

        return self.ds_dict[descriptor]['simulated_loc_m'], self.ds_dict[descriptor]['simulated_loc_product']



    def simulate_mismatch_measurements(self, SweepDataset, iterations=500, std_noise=None, noise_flag=True):
    ...
    def fit_measurement_gaussians(self, descriptor=None):
        ...
        for desc in descriptors:
            ...
            self.ds_dict[desc]["mismatch_gaussian_coeffs"] = mismatch_results["curvefit_coefficients"]
            self.ds_dict[desc]["mismatch_gaussian_cov"] = mismatch_results["curvefit_covariance"]

            loc_m = self.ds_dict[desc]["simulated_loc_m"]
            loc_m_results = self._fit_gaussians(loc_m, desc)
            self.ds_dict[desc]["loc_m_gaussian_coeffs"] = loc_m_results["curvefit_coefficients"]
            self.ds_dict[desc]["loc_m_gaussian_cov"] = loc_m_results["curvefit_covariance"]

            loc_product = self.ds_dict[desc]["simulated_loc_product"]
            loc_product_results = self._fit_gaussians(loc_product, desc)
            self.ds_dict[desc]["loc_product_gaussian_coeffs"] = loc_product_results["curvefit_coefficients"]
            self.ds_dict[desc]["loc_product_gaussian_cov"] = loc_product_results["curvefit_covariance"]
    ...
    def aggregate_fitted_gaussians(self, descriptor=None):
        ...
        for descriptor in descriptors:
            ...
            for string in ('mismatch', 'loc_m', 'loc_product'):
                ...
                self.ds_dict[descriptor][f"{string}_mean_opt_tor"] = mean_opt_tor.isel(param=0)
                self.ds_dict[descriptor][f"{string}_std_opt_tor"] = std_opt_tor.isel(param=0)
                self.ds_dict[descriptor][f"{string}_mean_delta_theta_m"] = mean_opt_tor.isel(param=1)
                self.ds_dict[descriptor][f"{string}_std_delta_theta_m"] = std_opt_tor.isel(param=1)

    ## Plot methods

    def plot_profile_comparisons(self, descriptor):
        mismatch_profile = self.ds_dict[descriptor]["mismatch_profile"].loc[{"poloidal_angle": self.poloidal_angles[0]}]
        loc_m_profile = self.ds_dict[descriptor]["loc_m_profile"].loc[{"poloidal_angle": self.poloidal_angles[0]}]
        loc_product_profile = self.ds_dict[descriptor]["loc_product_profile"].loc[{"poloidal_angle": self.poloidal_angles[0]}]
        ...
        for frequency in self.frequencies:
            ...
            ax.plot(toroidal_range, mismatch_profile[counter], color='k', label='mismatch formula')
            ax.plot(toroidal_range, scale_array(loc_m_profile[counter]), color='r', label='integrated loc_m')
            ax.plot(toroidal_range, scale_array(loc_product_profile[counter]), color='b', label='integrated loc_product')
    ...
    def plot_profile_fits(self, descriptor, sim_type='mismatch'):
        ...
        opt_tor = self.ds_dict[descriptor][f"{sim_type}_mean_opt_tor"].loc[{"poloidal_angle": self.poloidal_angles[0]}]
        delta = self.ds_dict[descriptor][f"{sim_type}_mean_delta_theta_m"].loc[{"poloidal_angle": self.poloidal_angles[0]}]
        ...
        for frequency in self.frequencies:
            ...
            ax.plot(toroidal_range, scale_array(lin_data), alpha=0.5, color=color, label='profile')
    ...
    def plot_opt_tor_hist(self, descriptor, sim_type='mismatch'):
        ...
        Returns:
            _type_: _description_
        """
        mean = self.ds_dict[descriptor][f"{sim_type}_mean_opt_tor"].loc[{"poloidal_angle": self.poloidal_angles[0]}]
        actual = self.ds_dict[descriptor]["opt_tor"].loc[{"poloidal_angle": self.poloidal_angles[0]}]
    ...
    def plot_opt_tor_vs_freq(self, descriptor, sim_type="mismatch"):
            mean = self.ds_dict[descriptor][f"{sim_type}_mean_opt_tor"].loc[{"poloidal_angle": self.poloidal_angles[0]}]
            std = self.ds_dict[descriptor][f"{sim_type}_std_opt_tor"].loc[{"poloidal_angle": self.poloidal_angles[0]}]
            actual = self.ds_dict[descriptor]["opt_tor"].loc[{"poloidal_angle": self.poloidal_angles[0]}]
