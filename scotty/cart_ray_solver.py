...
def make_solver_events(
    ...
    data_X_coord_min = field.X_coord.min()
    data_X_coord_max = field.X_coord.max()
    data_Y_coord_min = field.Y_coord.min()
    data_Y_coord_max = field.Y_coord.max()
    ...
    def event_leave_simulation(
        ...
        is_inside = (
            (q_X > data_X_coord_min)
            and (q_X < data_X_coord_max)
            and (q_Y > data_Y_coord_min)
            and (q_Y < data_Y_coord_max)
...
def handle_leaving_plasma_events(
    ...
    if detected("leave_plasma") and detected("leave_LCFS"):
        ...
        K_R_LCFS = ray_parameters_3D_events[0][2]
        if K_R_LCFS < 0:
