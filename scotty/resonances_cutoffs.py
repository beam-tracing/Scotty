...
from scotty.beam_me_up import make_density_fit, make_temperature_fit, create_magnetic_geometry
...
def resonances_cutoffs(
    ...
    def B_Total_R_Z(R_coord, Z_coord=0):
        return np.sqrt(field.B_R(R_coord, Z_coord)**2
    ...
    def gyro_freq_function(R_coord, Z_coord=0):
        ...
        gyro_freq = (
        constants.e * B_Total / electron_mass
    )
        return gyro_freq
    
    def X_mode_R_cutoff_function(R_coord, Z_coord=0):
        return 0.5*(gyro_freq_function(R_coord, Z_coord) 
                    + np.sqrt(gyro_freq_function(R_coord, Z_coord)**2
    ...
    def convert_to_GHz(function):
        return lambda R_coord, Z_coord=0: angular_frequency_to_freq_GHZ(function(R_coord, Z_coord))
