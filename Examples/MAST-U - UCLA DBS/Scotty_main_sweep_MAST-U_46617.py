...
poloidal_launch_angles_Torbeam = np.array([-2.5, -7.5])
# poloidal_launch_angles_Torbeam = np.array([-2.5])
# toroidal_launch_angles_Torbeam = np.array([2.0])
# toroidal_launch_angles_Torbeam = np.linspace(-10, 10, 21)
toroidal_launch_angles_Torbeam = np.linspace(2, 10, 9)
# launch_freqs_GHz = np.array([30.0])
# launch_freqs_GHz = np.array([32.5, 35, 37.5, 40, 42.5, 45, 47.5, 50])
launch_freqs_GHz = np.array([32.5, 40.0, 47.5])
        ...
        for kk, toroidal_launch_angle_Torbeam in enumerate(
            ...
            for ll, launch_freq_GHz in enumerate(launch_freqs_GHz):
                for mode_flag in [-1]:
                    ...
                    if equil_time == 0.1:
                        kwargs_dict["poloidal_flux_enter"] = 1.17671465**2
                    elif equil_time == 0.2:
                        kwargs_dict["poloidal_flux_enter"] = 1.06520296**2
                    elif equil_time == 0.22:
                        kwargs_dict["poloidal_flux_enter"] = 1.14032514**2
                    elif equil_time == 0.3:
                        kwargs_dict["poloidal_flux_enter"] = 1.13342726**2
                    elif equil_time == 0.4:
                        kwargs_dict["poloidal_flux_enter"] = 1.13342726**2
