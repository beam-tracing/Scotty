...
tor_launch_angles = np.flip(np.linspace(-4.0, 9.0, 131))
# launch_freqs_GHz = np.flip(np.array([55.0,57.5,60.0,62.5,65.0,67.5,70.0,72.5,75.0]))
launch_freqs_GHz = np.array([77.5])
...
for ii, launch_freq_GHz in enumerate(launch_freqs_GHz):
    for jj, pol_launch_angle in enumerate(pol_launch_angles):
        for kk, tor_launch_angle in enumerate(tor_launch_angles):
            ...
            kwargs_dict[
                "output_path"
            ] = "D:\\Dropbox\\VHChen2021\\Data - Scotty\\Run 11\\"
