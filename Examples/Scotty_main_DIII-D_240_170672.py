...
pol_launch_angles = np.array([-10.0])
# tor_launch_angles = np.linspace(-7.5,12.5,201)
# launch_freqs_GHz = np.array([55.0,57.5,60.0,62.5,65.0,67.5,70.0,72.5,75.0])
tor_launch_angles = np.array([2.5])
launch_freqs_GHz = np.array([55.0])
equil_time = 2000.0
...
for ii, launch_freq_GHz in enumerate(launch_freqs_GHz):
    for jj, pol_launch_angle in enumerate(pol_launch_angles):
        for kk, tor_launch_angle in enumerate(tor_launch_angles):
            ...
            kwargs_dict['poloidal_flux_enter'] = 1.19999993**2
            kwargs_dict['input_filename_suffix'] = '_170672_1200ms'
            kwargs_dict['output_path'] = os.path.dirname(os.path.abspath(__file__))  + '\\'
