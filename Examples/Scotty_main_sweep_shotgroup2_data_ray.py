...
q_R_all     = np.zeros([len(equil_times),len(mirror_tilts),len(mirror_rotations),len(launch_freqs_GHz)])
    ...
    for jj, mirror_tilt in enumerate(mirror_tilts):
        for kk, mirror_rotation in enumerate(mirror_rotations):
            for ll, launch_freq_GHz in enumerate(launch_freqs_GHz):
                ...
                kwargs_dict['input_filename_suffix'] = '_shotgroup2_avr_' + f'{equil_time*1000:.0f}' +'ms'
    
                if equil_time == 0.330:
                    kwargs_dict['poloidal_flux_enter'] = 1.10477199**2
...
np.savez(kwargs_dict['output_path'] + 'Cutoff_sweep_330ms',
