...
#        print(K_Z_launch)
    ...
    solver_ray_output = integrate.solve_ivp(
                        ray_evolution_2D_fun, [0,tau_max], ray_parameters_initial, 
    ...
    K_magnitude_ray = (  (np.real(ray_parameters[3,:]))**2 
                        + K_zeta_initial**2/(np.real(ray_parameters[0,:]))**2 
                        + (np.real(ray_parameters[4,:]))**2)**(0.5)
    ...
    """
    ray_parameters_initial_fine = ray_parameters[:,index_cutoff_estimate-1]
    ...
    solver_ray_output_fine = integrate.solve_ivp(
                        ray_evolution_2D_fun, [tau_start_fine,tau_end_fine], ray_parameters_initial_fine, 
                        method='RK45', t_eval=tau_points_fine, dense_output=False, 
                        events=event_leave_plasma, vectorized=False, args=solver_arguments
                    )
    ray_parameters_fine = solver_ray_output_fine.y  
    K_magnitude_ray_fine = (  (np.real(ray_parameters_fine[3,:]))**2 
                            + K_zeta_initial**2/(np.real(ray_parameters_fine[0,:]))**2 
                            + (np.real(ray_parameters_fine[4,:]))**2)**(0.5)
