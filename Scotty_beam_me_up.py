...
# , find_d2B_dR_dZ_FFD
from Scotty_fun_general import find_dB_dR_CFD, find_dB_dZ_CFD, find_d2B_dR2_CFD, find_d2B_dZ2_CFD
...
from Scotty_fun_general import f_ped
from Scotty_fun_general import find_quick_output
...
def beam_me_up(poloidal_launch_angle_Torbeam,
    ...
    if magnetic_data_path is None:
        if platform.system() == 'Windows':
            magnetic_data_path = os.path.dirname(
    ...
    #    ne_data_length = int(ne_data[0])
        ...
        interp_density_1D = interpolate.UnivariateSpline(ne_data_poloidal_flux_array, ne_data_density_array,
                                                         w=None, bbox=[None, None], 
                                                         k=interp_order, s=interp_smoothing, 
        ...
        def find_density_1D(poloidal_flux, poloidal_flux_enter=poloidal_flux_enter, density_fit_parameters=density_fit_parameters):
            a_height, a_width, a_exp, b_height, b_SOL, b_width, b_slope, b_pos = density_fit_parameters
            fp = f_ped(poloidal_flux, b_height, 0, b_width, b_slope, b_pos)
            density_fit = fp + (a_height - fp) * \
                np.exp(-poloidal_flux/a_width)**a_exp
            is_inside = poloidal_flux <= poloidal_flux_enter  # Boolean array
    ...
    if find_B_method == 'torbeam':
        ...
        with open(topfile_filename) as f:
            ...
            poloidalFlux_grid = read_floats_into_list_until(
                'you fall asleep', f)
        # ------------------------------
        ...
        data_B_R_grid = np.transpose((np.asarray(data_B_R_grid)).reshape(
            len(data_Z_coord), len(data_R_coord), order='C'))
        data_B_T_grid = np.transpose((np.asarray(data_B_T_grid)).reshape(
            len(data_Z_coord), len(data_R_coord), order='C'))
        data_B_Z_grid = np.transpose((np.asarray(data_B_Z_grid)).reshape(
            len(data_Z_coord), len(data_R_coord), order='C'))
        poloidalFlux_grid = np.transpose((np.asarray(poloidalFlux_grid)).reshape(
            len(data_Z_coord), len(data_R_coord), order='C'))
        # -------------------

        # Interpolation functions declared
        interp_B_R = interpolate.RectBivariateSpline(data_R_coord, data_Z_coord, data_B_R_grid, bbox=[
                                                     None, None, None, None], kx=interp_order, ky=interp_order, s=interp_smoothing)
        interp_B_T = interpolate.RectBivariateSpline(data_R_coord, data_Z_coord, data_B_T_grid, bbox=[
                                                     None, None, None, None], kx=interp_order, ky=interp_order, s=interp_smoothing)
        interp_B_Z = interpolate.RectBivariateSpline(data_R_coord, data_Z_coord, data_B_Z_grid, bbox=[
                                                     None, None, None, None], kx=interp_order, ky=interp_order, s=interp_smoothing)
        ...
        interp_poloidal_flux = interpolate.RectBivariateSpline(data_R_coord, data_Z_coord, poloidalFlux_grid, bbox=[
                                                               None, None, None, None], kx=interp_order, ky=interp_order, s=interp_smoothing)
        ...
        def find_B_p(q_R, q_Z,
                     R_axis, minor_radius_a, B_p_a):
            B_p = B_p_a * (np.sqrt((q_R-R_axis)**2 + q_Z**2) / minor_radius_a)
            return B_p

        def find_B_R(q_R, q_Z,
                     R_axis=R_axis, minor_radius_a=minor_radius_a, B_p_a=B_p_a):
        ...
        def interp_poloidal_flux(q_R, q_Z,
                                 R_axis=R_axis, minor_radius_a=minor_radius_a, grid=None):
    ...
    #        input_group = dataset.groups['input']
            ...
            unnormalised_poloidalFlux_grid = profiles2D.variables['poloidalFlux'][time_idx][:][:]
            ...
            unnormalizedPoloidalFlux = fluxFunctionProfiles.variables['poloidalFlux'][time_idx][:]
            ...
            [polflux_const_m, polflux_const_c] = np.polyfit(
                unnormalizedPoloidalFlux, poloidalFlux, 1)  # linear fit
            poloidalFlux_grid = unnormalised_poloidalFlux_grid * \
                polflux_const_m + polflux_const_c

        elif find_B_method == 'UDA_saved' and shot is not None and shot <= 30471:  # MAST
            print(shot)
            # 30471 is the last shot on MAST
            # data saved differently for MAST-U shots
            loadfile = np.load(magnetic_data_path +
                               str(shot) + '_equilibrium_data.npz')
            ...
            # On time base B
            unnormalised_poloidalFlux_grid_all_times = loadfile['poloidal_flux_unnormalised']
            loadfile.close()
            ...
            poloidalFlux_grid = (
                unnormalised_poloidalFlux_grid - polflux_axis)/(polflux_boundary-polflux_axis)

        elif find_B_method == 'UDA_saved' and shot is not None and shot > 30471:  # MAST-U
            # 30471 is the last shot on MAST
            # data saved differently for MAST-U shots
            loadfile = np.load(magnetic_data_path +
                               str(shot) + '_equilibrium_data.npz')
            ...
            poloidalFlux_unnormalised_boundary = poloidalFlux_unnormalised_boundary_all_times[
                t_idx]

            polflux_const_m = (
                1.0 - 0.0) / (poloidalFlux_unnormalised_boundary-poloidalFlux_unnormalised_axis)
            ...
            poloidalFlux_unnormalised_boundary = loadfile['poloidalFlux_unnormalised_boundary']
            loadfile.close()
        ...
        #                                     assume_sorted=False)
        interp_rBphi = interpolate.UnivariateSpline(poloidalFlux, rBphi,
                                                    w=None, bbox=[None, None], 
                                                    k=interp_order, s=interp_smoothing, 
                                                    ext=0, check_finite=False)
        ...
        rBphi_gradient_for_extrapolation = (
            rBphi[-1] - rBphi[-2])/(poloidalFlux[-1] - poloidalFlux[-2])
        last_poloidal_flux = poloidalFlux[-1]
        last_rBphi = rBphi[-1]

        def find_B_R(q_R, q_Z, delta_Z=delta_Z, interp_poloidal_flux=interp_poloidal_flux, polflux_const_m=polflux_const_m):
            dpolflux_dZ = find_dpolflux_dZ(
                q_R, q_Z, delta_Z, interp_poloidal_flux)
        ...
        def find_B_Z(q_R, q_Z, delta_R=delta_R, interp_poloidal_flux=interp_poloidal_flux, polflux_const_m=polflux_const_m):
            dpolflux_dR = find_dpolflux_dR(
                q_R, q_Z, delta_R, interp_poloidal_flux)
    ...
    if vacuumLaunch_flag:
        ...
        K_R_launch = -wavenumber_K0 * np.cos(toroidal_launch_angle_Torbeam/180.0*math.pi) * np.cos(
            poloidal_launch_angle_Torbeam/180.0*math.pi)  # K_R
        K_zeta_launch = -wavenumber_K0 * np.sin(toroidal_launch_angle_Torbeam/180.0*math.pi) * np.cos(
            poloidal_launch_angle_Torbeam/180.0*math.pi) * launch_position[0]  # K_zeta
        ...
        Psi_w_beam_launch_cartersian = np.array(
            [
                [wavenumber_K0*launch_beam_curvature +
                    2j*launch_beam_width**(-2), 0],
                [0, wavenumber_K0*launch_beam_curvature +
        ...
        if vacuum_propagation_flag:
            ...
            # Finds entry point
            search_Z_end = launch_position[2] - launch_position[0] * \
                np.tan(np.radians(poloidal_launch_angle_Torbeam))
            ...
            poloidal_flux_coarse_search_array = np.zeros(
                numberOfCoarseSearchPoints)
            ...
            R_fine_search_array = np.linspace(
                launch_position[0], R_coarse_search_array[first_inside_index], numberOfFineSearchPoints)
            Z_fine_search_array = np.linspace(
                launch_position[2], Z_coarse_search_array[first_inside_index], numberOfFineSearchPoints)
            ...
            entry_index = find_nearest(
                poloidal_fine_search_array, poloidal_flux_enter)
            ...
            entry_position[1] = K_zeta_launch/K_R_launch * \
            ...
            distance_from_launch_to_entry = np.sqrt(
                launch_position[0]**2
                + entry_position[0]**2
                - 2 * launch_position[0] * entry_position[0] *
            ...
            K_lab_Cartesian_launch = find_K_lab_Cartesian(
                K_lab_launch, launch_position)
            K_lab_Cartesian_entry = K_lab_Cartesian_launch
            entry_position_Cartesian = find_q_lab_Cartesian(entry_position)
            K_lab_entry = find_K_lab(
                K_lab_Cartesian_entry, entry_position_Cartesian)

            K_R_entry = K_lab_entry[0]  # K_R
            K_zeta_entry = K_lab_entry[1]
            K_Z_entry = K_lab_entry[2]  # K_z

            Psi_w_beam_inverse_entry_cartersian = distance_from_launch_to_entry / \
                (wavenumber_K0)*identity_matrix_2D + \
            ...
            Psi_3D_beam_entry_cartersian = np.array([
                [Psi_w_beam_entry_cartersian[0][0],
                    Psi_w_beam_entry_cartersian[0][1], 0],
                [Psi_w_beam_entry_cartersian[1][0],
                    Psi_w_beam_entry_cartersian[1][1], 0],
                [0, 0, 0]
            ])  # 'entry' is still in vacuum, so the components of Psi along g are all 0 (since \nabla H = 0)

            Psi_3D_lab_entry_cartersian = np.matmul(rotation_matrix_inverse, np.matmul(
                Psi_3D_beam_entry_cartersian, rotation_matrix))

            # Convert to cylindrical coordinates
            Psi_3D_lab_entry = find_Psi_3D_lab(
                Psi_3D_lab_entry_cartersian, entry_position[0], entry_position[1], K_R_entry, K_zeta_entry)
            ...
            if Psi_BC_flag:  # Use BCs
                ...
                d_poloidal_flux_d_R_boundary = find_d_poloidal_flux_dR(
                    initial_position[0], initial_position[2], delta_R, interp_poloidal_flux)
        ...
        Psi_3D_lab_initial = find_Psi_3D_lab(
            plasmaLaunch_Psi_3D_lab_Cartesian, launch_position[0], plasmaLaunch_K[0], plasmaLaunch_K[1])
        K_R_initial = K_R_launch
        K_zeta_initial = K_zeta_launch
        K_Z_initial = K_Z_launch
        initial_position = launch_position
...
#        print(K_Z_launch)
    ...
    beam_parameters_initial[6] = np.real(
        Psi_3D_lab_initial[1, 1])  # Psi_zetazeta
    ...
    beam_parameters_initial[10] = np.real(
        Psi_3D_lab_initial[1, 2])  # Psi_zetaZ

    beam_parameters_initial[11] = np.imag(Psi_3D_lab_initial[0, 0])  # Psi_RR
    beam_parameters_initial[12] = np.imag(
        Psi_3D_lab_initial[1, 1])  # Psi_zetazeta
    beam_parameters_initial[13] = np.imag(Psi_3D_lab_initial[2, 2])  # Psi_ZZ
    beam_parameters_initial[14] = np.imag(
        Psi_3D_lab_initial[0, 1])  # Psi_Rzeta
    beam_parameters_initial[15] = np.imag(Psi_3D_lab_initial[0, 2])  # Psi_RZ
    beam_parameters_initial[16] = np.imag(
        Psi_3D_lab_initial[1, 2])  # Psi_zetaZ

    ray_parameters_initial = np.real(beam_parameters_initial[0:5])
    ray_parameters_2D_initial = np.delete(
        ray_parameters_initial, 1)  # Remove q_zeta
    # -------------------
    ...
    def event_leave_simulation(tau, ray_parameters_2D, K_zeta,
        ...
        is_inside = (q_R > data_R_coord_min) and (q_R < data_R_coord_max) and (
            q_Z > data_Z_coord_min) and (q_Z < data_Z_coord_max)
    ...
    def event_cross_resonance(tau, ray_parameters_2D, K_zeta,
                              launch_angular_frequency, mode_flag,
                              delta_R, delta_Z, delta_K_R, delta_K_zeta, delta_K_Z,
                              interp_poloidal_flux, find_density_1D,
                              find_B_R, find_B_T, find_B_Z
    ...
    def event_reach_cutoff(tau, ray_parameters_2D, K_zeta,
                           launch_angular_frequency, mode_flag,
                           delta_R, delta_Z, delta_K_R, delta_K_zeta, delta_K_Z,
                           interp_poloidal_flux, find_density_1D,
                           find_B_R, find_B_T, find_B_Z
    ...
    solver_ray_events = (event_leave_plasma,
                         event_leave_LCFS,
    ...
    solver_ray_output = integrate.solve_ivp(
        ...
        rtol=rtol, atol=atol, max_step=50
    ...
    if quick_run:
        ...
        for ii in range(0,numTurningPoints):
            ...
            K_turning_pt[ii] = np.linalg.norm(np.array([K_R,K_zeta_initial/q_R,K_Z]))
        ...
        sin_theta_m_cutoff = np.dot(B_cutoff, K_cutoff) / (np.linalg.norm(K_cutoff) * np.linalg.norm(B_cutoff))
        # Assumes the mismatch angle is never smaller than -90deg or bigger than 90deg
        theta_m_cutoff = np.sign(sin_theta_m_cutoff) * np.arcsin(abs(sin_theta_m_cutoff))
        
        poloidal_flux_cutoff = interp_poloidal_flux(q_R_cutoff, q_Z_cutoff)
        
        return q_R_cutoff, q_Z_cutoff, np.linalg.norm(K_cutoff), poloidal_flux_cutoff, theta_m_cutoff
    ...
    if (len(tau_events[3]) == 0):
        ...
        K_magnitude_ray = ((np.real(ray_parameters_2D[2, :max_tau_idx]))**2
                           + K_zeta_initial**2 /
                           (np.real(ray_parameters_2D[0, :max_tau_idx]))**2
                           + (np.real(ray_parameters_2D[3, :max_tau_idx]))**2)**(0.5)

        index_cutoff_estimate = K_magnitude_ray.argmin()

        ray_parameters_2D_initial_fine = ray_parameters_2D[:, max(
            0, index_cutoff_estimate-1)]
    ...
    Psi_3D_output[:, 0, 0] = beam_parameters[5, :] + \
        1j*beam_parameters[11, :]  # d (Psi_RR) / d tau
    Psi_3D_output[:, 1, 1] = beam_parameters[6, :] + 1j * \
        beam_parameters[12, :]  # d (Psi_zetazeta) / d tau
    Psi_3D_output[:, 2, 2] = beam_parameters[7, :] + \
        1j*beam_parameters[13, :]  # d (Psi_ZZ) / d tau
    Psi_3D_output[:, 0, 1] = beam_parameters[8, :] + 1j * \
        beam_parameters[14, :]  # d (Psi_Rzeta) / d tau
    Psi_3D_output[:, 0, 2] = beam_parameters[9, :] + \
        1j*beam_parameters[15, :]  # d (Psi_RZ) / d tau
    Psi_3D_output[:, 1, 2] = beam_parameters[10, :] + \
        1j*beam_parameters[16, :]  # d (Psi_zetaZ) / d tau
    ...
    poloidal_flux_output = interp_poloidal_flux(
        q_R_array, q_Z_array, grid=False)
    ...
    g_magnitude_output = (q_R_array**2 * dH_dKzeta_output **
                          2 + dH_dKR_output**2 + dH_dKZ_output**2)**0.5
    g_hat_output[:, 0] = dH_dKR_output / g_magnitude_output  # g_hat_R
    g_hat_output[:, 1] = q_R_array * dH_dKzeta_output / \
        g_magnitude_output  # g_hat_zeta
    ...
    dbhat_dR = find_dbhat_dR(q_R_array, q_Z_array,
                             delta_R, find_B_R, find_B_T, find_B_Z)
    dbhat_dZ = find_dbhat_dZ(q_R_array, q_Z_array,
                             delta_Z, find_B_R, find_B_T, find_B_Z)
    ...
    # x_hat and y_hat
    y_hat_output = make_unit_vector_from_cross_product(
        b_hat_output, g_hat_output)
    x_hat_output = make_unit_vector_from_cross_product(
        y_hat_output, g_hat_output)
    ...
    # -------------------
    #
    H_output = find_H(q_R_array, q_Z_array, K_R_array, K_zeta_initial, K_Z_array,
                      launch_angular_frequency, mode_flag,
                      interp_poloidal_flux, find_density_1D, find_B_R, find_B_T, find_B_Z)
    H_other = find_H(q_R_array, q_Z_array, K_R_array, K_zeta_initial, K_Z_array,
                     launch_angular_frequency, -mode_flag,
                     interp_poloidal_flux, find_density_1D, find_B_R, find_B_T, find_B_Z)

    # nabla H along the ray
    dH_dR_output = find_dH_dR(q_R_array, q_Z_array, K_R_array, K_zeta_initial, K_Z_array,
                              launch_angular_frequency, mode_flag,
                              delta_R,
                              interp_poloidal_flux, find_density_1D,
                              find_B_R, find_B_T, find_B_Z
                              )
    dH_dZ_output = find_dH_dZ(q_R_array, q_Z_array, K_R_array, K_zeta_initial, K_Z_array,
                              launch_angular_frequency, mode_flag,
                              delta_Z,
                              interp_poloidal_flux, find_density_1D,
                              find_B_R, find_B_T, find_B_Z
    ...
    point_spacing = ((np.diff(q_X_array))**2 +
                     (np.diff(q_Y_array))**2 + (np.diff(q_Z_array))**2)**0.5
    ...
    K_magnitude_array = (K_R_array**2 + K_zeta_initial **
                         2/q_R_array**2 + K_Z_array**2)**(0.5)
    ...
    cyclotron_freq_output = launch_angular_frequency * \
        find_normalised_gyro_freq(B_magnitude, launch_angular_frequency)
...
#     out_index = cutoff_index + find_nearest(poloidal_flux_b,in_out_poloidal_flux)
    ...
    sin_theta_m_analysis[:] = (b_hat_output[:, 0]*K_R_array[:] + b_hat_output[:, 1]*K_zeta_initial/q_R_array[:] +
                               b_hat_output[:, 2]*K_Z_array[:]) / (K_magnitude_array[:])  # B \cdot K / (abs (B) abs(K))

    # Assumes the mismatch angle is never smaller than -90deg or bigger than 90deg
    theta_m_output = np.sign(sin_theta_m_analysis) * \
        np.arcsin(abs(sin_theta_m_analysis))

    kperp1_hat_output = make_unit_vector_from_cross_product(
        y_hat_output, b_hat_output)
    ...
    theta_output = np.sign(sin_theta_analysis) * \
        np.arcsin(abs(sin_theta_analysis))
    ...
    k_perp_1_bs = -2 * K_magnitude_array * \
        np.cos(theta_m_output + theta_output) / cos_theta_analysis
    ...
    y_hat_Cartesian[:, 0] = y_hat_output[:, 0] * \
        np.cos(q_zeta_array) - y_hat_output[:, 1]*np.sin(q_zeta_array)
    y_hat_Cartesian[:, 1] = y_hat_output[:, 0] * \
        np.sin(q_zeta_array) + y_hat_output[:, 1]*np.cos(q_zeta_array)
    y_hat_Cartesian[:, 2] = y_hat_output[:, 2]
    x_hat_Cartesian[:, 0] = x_hat_output[:, 0] * \
        np.cos(q_zeta_array) - x_hat_output[:, 1]*np.sin(q_zeta_array)
    x_hat_Cartesian[:, 1] = x_hat_output[:, 0] * \
        np.sin(q_zeta_array) + x_hat_output[:, 1]*np.cos(q_zeta_array)
    ...
    Psi_xx_entry = np.dot(x_hat_Cartesian[0, :], np.dot(
        Psi_3D_lab_entry_cartersian, x_hat_Cartesian[0, :]))
    Psi_xy_entry = np.dot(x_hat_Cartesian[0, :], np.dot(
        Psi_3D_lab_entry_cartersian, y_hat_Cartesian[0, :]))
    Psi_yy_entry = np.dot(y_hat_Cartesian[0, :], np.dot(
        Psi_3D_lab_entry_cartersian, y_hat_Cartesian[0, :]))
    ...
    ray_curvature_kappa_output[:, 2] = (
        1/g_magnitude_output) * np.gradient(g_hat_output[:, 2], tau_array)
    ...
    d_xhat_d_tau_output[:, 0] = np.gradient(
        x_hat_output[:, 0], tau_array) - x_hat_output[:, 1]*dH_dKzeta_output  # See notes 07 June 2021
    d_xhat_d_tau_output[:, 1] = np.gradient(
        x_hat_output[:, 1], tau_array) + x_hat_output[:, 0]*dH_dKzeta_output  # See notes 07 June 2021
    ...
    M_xx_output = Psi_xx_output \
                  + (k_perp_1_bs/2) * xhat_dot_grad_bhat_dot_ghat_output
    M_xy_output = Psi_xy_output \
                  + (k_perp_1_bs/2) * yhat_dot_grad_bhat_dot_ghat_output
    M_yy_output = Psi_yy_output
    # -----
    ...
    delta_theta_m = np.sqrt(
        np.imag(M_w_inv_yy_output) / ((np.imag(M_w_inv_xy_output)) **
                                      2 - np.imag(M_w_inv_xx_output)*np.imag(M_w_inv_yy_output))
    ) / (K_magnitude_array)
    ...
    print('mismatch attenuation', np.exp(-2 *
          (theta_m_output[cutoff_index]/delta_theta_m[cutoff_index])**2))
    # -----
...
#    print(cutoff_index)
    ...
    poloidal_flux_on_midplane = interp_poloidal_flux(
        R_midplane_points, 0, grid=False)  # poloidal flux at R and z=0
    ...
    K_magnitude_array_plus_KR = np.sqrt(
        (K_R_array+delta_K_R)**2 + K_Z_array**2 + K_zeta_initial**2/q_R_array**2)
    K_magnitude_array_minus_KR = np.sqrt(
        (K_R_array-delta_K_R)**2 + K_Z_array**2 + K_zeta_initial**2/q_R_array**2)
    K_magnitude_array_plus_Kzeta = np.sqrt(
        K_R_array**2 + K_Z_array**2 + (K_zeta_initial+delta_K_zeta)**2/q_R_array**2)
    K_magnitude_array_minus_Kzeta = np.sqrt(
        K_R_array**2 + K_Z_array**2 + (K_zeta_initial-delta_K_zeta)**2/q_R_array**2)
    K_magnitude_array_plus_KZ = np.sqrt(
        K_R_array**2 + (K_Z_array+delta_K_Z)**2 + K_zeta_initial**2/q_R_array**2)
    K_magnitude_array_minus_KZ = np.sqrt(
        K_R_array**2 + (K_Z_array-delta_K_Z)**2 + K_zeta_initial**2/q_R_array**2)

    H_1_Cardano_array, H_2_Cardano_array, H_3_Cardano_array = find_H_Cardano(
        K_magnitude_array, launch_angular_frequency, epsilon_para_output, epsilon_perp_output, epsilon_g_output, theta_m_output)
    ...
    if mode_flag == 1:
        _, _, H_Cardano_plus_KR_array = find_H_Cardano(
            K_magnitude_array_plus_KR, launch_angular_frequency, epsilon_para_output, epsilon_perp_output, epsilon_g_output, theta_m_output)
        _, _, H_Cardano_minus_KR_array = find_H_Cardano(
            K_magnitude_array_minus_KR, launch_angular_frequency, epsilon_para_output, epsilon_perp_output, epsilon_g_output, theta_m_output)
        _, _, H_Cardano_plus_Kzeta_array = find_H_Cardano(
            K_magnitude_array_plus_Kzeta, launch_angular_frequency, epsilon_para_output, epsilon_perp_output, epsilon_g_output, theta_m_output)
        _, _, H_Cardano_minus_Kzeta_array = find_H_Cardano(
            K_magnitude_array_minus_Kzeta, launch_angular_frequency, epsilon_para_output, epsilon_perp_output, epsilon_g_output, theta_m_output)
        _, _, H_Cardano_plus_KZ_array = find_H_Cardano(
            K_magnitude_array_plus_KZ, launch_angular_frequency, epsilon_para_output, epsilon_perp_output, epsilon_g_output, theta_m_output)
        _, _, H_Cardano_minus_KZ_array = find_H_Cardano(
            K_magnitude_array_minus_KZ, launch_angular_frequency, epsilon_para_output, epsilon_perp_output, epsilon_g_output, theta_m_output)
    elif mode_flag == -1:
    ...
    g_R_Cardano = np.real(H_Cardano_plus_KR_array -
                          H_Cardano_minus_KR_array) / (2 * delta_K_R)
    g_zeta_Cardano = np.real(
        H_Cardano_plus_Kzeta_array - H_Cardano_minus_Kzeta_array) / (2 * delta_K_zeta)
    g_Z_Cardano = np.real(H_Cardano_plus_KZ_array -
                          H_Cardano_minus_KZ_array) / (2 * delta_K_Z)
    ...
    loc_r = (2 * constants.c / launch_angular_frequency)**2 / \
        g_magnitude_Cardano**2

    # Spectrum piece of localisation as a function of distance along ray
    spectrum_power_law_coefficient = 13/3  # Turbulence cascade
    loc_s = (k_perp_1_bs / (-2*wavenumber_K0)
             )**(-spectrum_power_law_coefficient)

    # Beam piece of localisation as a function of distance along ray
    det_imag_Psi_w_analysis = np.imag(Psi_xx_output)*np.imag(Psi_yy_output) - np.imag(
        Psi_xy_output)**2  # Determinant of the imaginary part of Psi_w
    # Determinant of the real part of Psi_w. Not needed for the calculation, but gives useful insight
    det_real_Psi_w_analysis = np.real(
        Psi_xx_output)*np.real(Psi_yy_output) - np.real(Psi_xy_output)**2

    # Assumes circular beam at launch
    beam_waist_y = find_waist(
        launch_beam_width, wavenumber_K0, launch_beam_curvature)

    loc_b = (beam_waist_y/np.sqrt(2)) * det_imag_Psi_w_analysis / \
    ...
    epsilon_minus_identity = np.zeros(
        [numberOfDataPoints, 3, 3], dtype='complex128')
    epsilon_minus_identity[:, 0, 0] = epsilon_perp_output - \
        np.ones(numberOfDataPoints)
    epsilon_minus_identity[:, 1, 1] = epsilon_perp_output - \
        np.ones(numberOfDataPoints)
    epsilon_minus_identity[:, 2, 2] = epsilon_para_output - \
        np.ones(numberOfDataPoints)
    ...
    loc_p_unnormalised = np.divide(abs(contract_special(np.conjugate(e_hat_output), contract_special(epsilon_minus_identity, e_hat_output)))**2,
                                   (electron_density_output*10**19)**2,
                                   out=np.zeros_like(electron_density_output),
                                   where=electron_density_output > 1e-6)
    loc_p = (launch_angular_frequency**2 * constants.epsilon_0 *
             constants.m_e / constants.e**2)**2 * loc_p_unnormalised
    ...
    l_lc = distance_along_line - \
        distance_along_line[cutoff_index]  # Distance from cutoff
    ...
    if detailed_analysis_flag and (cutoff_index+1 != len(tau_array)):
        ...
        loc_b_r_s_delta_l = np.array(
            [loc_b_r_s_delta_l_1, loc_b_r_s_delta_l_2])
        ...
        cum_loc_b_r_s = integrate.cumtrapz(
            loc_b_r_s, distance_along_line, initial=0)
        cum_loc_b_r_s = (cum_loc_b_r_s - max(cum_loc_b_r_s)/2)
        cum_loc_b_r = integrate.cumtrapz(
            loc_b_r, distance_along_line, initial=0)
        ...
        cum_loc_b_r_s_half_width = (
            cum_loc_b_r_s_delta_l_2 - cum_loc_b_r_s_delta_l_1)/2
        cum_loc_b_r_delta_l_1 = find_x0(
            l_lc, cum_loc_b_r, -cum_loc_b_r_max_over_e2)
        cum_loc_b_r_delta_l_2 = find_x0(
            l_lc, cum_loc_b_r,  cum_loc_b_r_max_over_e2)
        cum_loc_b_r_delta_l = np.array(
            [cum_loc_b_r_delta_l_1, cum_loc_b_r_delta_l_2])
        cum_loc_b_r_half_width = (
            cum_loc_b_r_delta_l_2 - cum_loc_b_r_delta_l_1)/2

        # Gives the inter-e2 range (analogous to interquartile range) in kperp1.
        # Bear in mind that since abs(kperp1) is minimised at cutoff, one really has to use that in addition to these.
        cum_loc_b_r_s_delta_kperp1_1 = find_x0(
            k_perp_1_bs[0:cutoff_index], cum_loc_b_r_s[0:cutoff_index], -cum_loc_b_r_s_max_over_e2)
        cum_loc_b_r_s_delta_kperp1_2 = find_x0(
            k_perp_1_bs[cutoff_index::], cum_loc_b_r_s[cutoff_index::],  cum_loc_b_r_s_max_over_e2)
        ...
        cum_loc_b_r_delta_kperp1_1 = find_x0(
            k_perp_1_bs[0:cutoff_index], cum_loc_b_r[0:cutoff_index], -cum_loc_b_r_max_over_e2)
        cum_loc_b_r_delta_kperp1_2 = find_x0(
            k_perp_1_bs[cutoff_index::], cum_loc_b_r[cutoff_index::],  cum_loc_b_r_max_over_e2)
        ...
        loc_b_r_s_max_l_lc = distance_along_line[loc_b_r_s_max_index] - \
            distance_along_line[cutoff_index]
        loc_b_r_max_index = find_nearest(loc_b_r, loc_b_r.max())
        loc_b_r_max_l_lc = distance_along_line[loc_b_r_max_index] - \
            distance_along_line[cutoff_index]

        # Gives the mean l-lc for backscattering
        cum_loc_b_r_s_mean_l_lc = np.trapz(loc_b_r_s*distance_along_line, distance_along_line) / np.trapz(
            loc_b_r_s, distance_along_line) - distance_along_line[cutoff_index]
        cum_loc_b_r_mean_l_lc = np.trapz(loc_b_r*distance_along_line, distance_along_line) / np.trapz(
            loc_b_r, distance_along_line) - distance_along_line[cutoff_index]
        ...
        cum_loc_b_r_s_mean_kperp1 = np.trapz(
            loc_b_r_s*k_perp_1_bs, k_perp_1_bs) / np.trapz(loc_b_r_s, k_perp_1_bs)
        cum_loc_b_r_mean_kperp1 = np.trapz(
            loc_b_r * k_perp_1_bs, k_perp_1_bs) / np.trapz(loc_b_r, k_perp_1_bs)
...
#     # -------------------
    ...
    np.savez(output_path + 'analysis_output' + output_filename_suffix,
             ...
             # The 1/e2 distances,  (l - l_c)
             loc_b_r_s_delta_l=loc_b_r_s_delta_l, loc_b_r_delta_l=loc_b_r_delta_l,
             # The 1/e2 distances, kperp1, estimated from (l - l_c)
             loc_b_r_s_delta_kperp1=loc_b_r_s_delta_kperp1, loc_b_r_delta_kperp1=loc_b_r_delta_kperp1,
             ...
             cum_loc_b_r_s_delta_l=cum_loc_b_r_s_delta_l, cum_loc_b_r_delta_l=cum_loc_b_r_delta_l,
             # The cumloc 1/e2 distances, kperp1
             cum_loc_b_r_s_delta_kperp1=cum_loc_b_r_s_delta_kperp1, cum_loc_b_r_delta_kperp1=cum_loc_b_r_delta_kperp1,
    ...
    if figure_flag:
        ...
        CS = plt.contour(data_R_coord, data_Z_coord, np.transpose(
            poloidalFlux_grid), contour_levels, vmin=0, vmax=1, cmap='plasma_r')
        plt.clabel(CS, inline=1, fontsize=10)  # Labels the flux surfaces
        plt.plot(
            np.concatenate(
                [[launch_position[0], initial_position[0]], q_R_array]),
            np.concatenate(
                [[launch_position[2], initial_position[2]], q_Z_array]),
