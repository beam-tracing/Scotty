...
def find_x0(xs,ys,y0): 
    ...
    #
    # Note that while xs must be 1D, ys can be N-dimensional, hence, the axis
    # argument is required to specify which axis along ys to be interpolated
    #
...
#
# Definition of find_density_1D and find_poloidal_flux still unclear for now
def find_H(q_X, q_Y, q_Z, K_X, K_Y, K_Z, launch_angular_frequency, mode_flag, 
           find_poloidal_flux, find_density_1D, find_B_X, 
           find_B_Y, find_B_Z): 
    """
    DESCRIPTION
    ==============================
    ...
    - Changed the arguments of find_poloidal_flux from "q_R, q_Z" to 
      "q_X, q_Y, q_Z".
    ...
    # Note that our interpolator is DIFFERENT to the original Scotty, we are using
    # RegularGridInterpolator instead of RectBivariateSpline
    poloidal_flux = find_poloidal_flux(q_X, q_Y, q_Z)
    electron_density = find_density_1D(poloidal_flux)
    
    ## DEBUGGING
    # print('poloidal_flux = ' + str(poloidal_flux))
    # print('electron_density = ' + str(electron_density))
    # print('position = ' + str((q_X, q_Y, q_Z)))
...
def find_d_poloidal_flux_dX(q_X, q_Y, q_Z, delta_X, find_poloidal_flux):
    """
    DESCRIPTION
    ==============================
    ...
    find_poloidal_flux (function):
        calculates the poloidal flux for a given position (q_X, q_Y, q_Z)
    ...
    poloidal_flux_0 = find_poloidal_flux(q_X, q_Y, q_Z)
    poloidal_flux_1 = find_poloidal_flux(q_X + delta_X, q_Y, q_Z)
    poloidal_flux_2 = find_poloidal_flux(q_X + 2*delta_X, q_Y, q_Z)
...
def find_d_poloidal_flux_dY(q_X, q_Y, q_Z, delta_Y, find_poloidal_flux):
    """
    DESCRIPTION
    ==============================
    ...
    # We are using RegularGridInterpolator instead of RectBivariateSpline
    poloidal_flux_0 = find_poloidal_flux(q_X, q_Y, q_Z)
    poloidal_flux_1 = find_poloidal_flux(q_X , q_Y + delta_Y, q_Z)
    poloidal_flux_2 = find_poloidal_flux(q_X , q_Y + 2*delta_Y, q_Z)
...
def find_d_poloidal_flux_dZ(q_X, q_Y, q_Z, delta_Z, find_poloidal_flux):
    ...
    # We are using RegularGridInterpolator instead of RectBivariateSpline
    poloidal_flux_0 = find_poloidal_flux(q_X, q_Y, q_Z)
    poloidal_flux_1 = find_poloidal_flux(q_X , q_Y, q_Z + delta_Z)
    poloidal_flux_2 = find_poloidal_flux(q_X , q_Y, q_Z + 2*delta_Z)
...
def find_d2_poloidal_flux_dX2_FFD(q_X, q_Y, q_Z, delta_X, 
    ...
    find_poloidal_flux (function):
        calculates the value of the poloidal flux given a position (q_X, q_Y, q_Z)
    ...
    # n+1 step of the poloidal flux 
    poloidal_flux_3 = find_poloidal_flux(q_X + 3*delta_X, q_Y, q_Z)
...
def find_d2_poloidal_flux_dY2_FFD(q_X, q_Y, q_Z, delta_Y, 
    ...
    # n+1 step of the poloidal flux 
    poloidal_flux_3 = find_poloidal_flux(q_X, q_Y + 3*delta_Y, q_Z)
...
def find_d2_poloidal_flux_dZ2_FFD(q_X, q_Y, q_Z, delta_Z, 
    ...
    # n+1 step of the poloidal flux 
    poloidal_flux_3 = find_poloidal_flux(q_X, q_Y, q_Z + 3*delta_Z)
